...
public class RemeberTheBigWebAction extends JSONPrintAction{
	 ...
	 * @return
	 */
	public String processsQueryRemeberTheBigAction(){
		Pagination pages = DaoFactory.getModelRemeberTheBigDao().queryModelRemeberTheBigPage(columnsType,new Integer(1), pageMaxSize, pageNumber);
	...
	public String processsQueryRemeberBigAction(){
		List imageList = DaoFactory.getModelColumnImageDao().queryModelColumnImageBy(imageType, imageLimit);
		super.setRequestAttribute("imageList", imageList);
		List remberList = DaoFactory.getModelRemeberTheBigDao().queryModelRemeberTheBigList(null, remberColumnsType, new Integer(1));
	 ...
	 * @return
	 */
	public String 	processsFactoryEnvironmentAction(){
		List imageList = DaoFactory.getModelColumnImageDao().queryModelColumnImageBy(imageType, imageLimit);
		super.setRequestAttribute("imageList", imageList);
		Pagination pages = DaoFactory.getModelFactoryEnvironmentDao().queryModelFactoryEnvironmentPage(null,new Integer(1),pageFactoryMaxSize, pageNumber);
		super.setRequestAttribute("pages", pages);
		return SUCCESS;	
	}
	
	private ModelFactoryEnvironment factory= new ModelFactoryEnvironment();
	private Integer factoryId;
	/**
	 * 厂区环境详细
	 * @return
	 */
	public String 	processsFactoryEnvironmentDetailAction(){
		List imageList = DaoFactory.getModelColumnImageDao().queryModelColumnImageBy(imageType, imageLimit);
		super.setRequestAttribute("imageList", imageList);
		Map  oneMap = null;//保存下标为0的Map
		Map twoMap = null;//保存当前下标的Map
		List list = DaoFactory.getModelFactoryEnvironmentDao().queryModelFactoryEnvironmentWebList();
		...
		factory=DaoFactory.getModelFactoryEnvironmentDao().queryModelFactoryEnvironmentById(factoryId);
	 ...
	 * @return
	 */
	public String 	processsContactAction(){
		List imageList = DaoFactory.getModelColumnImageDao().queryModelColumnImageBy(imageType, imageLimit);
		super.setRequestAttribute("imageList", imageList);
		return SUCCESS;	
	}
	...
	public ModelFactoryEnvironment getFactory() {
		return factory;
	}


	public void setFactory(ModelFactoryEnvironment factory) {
