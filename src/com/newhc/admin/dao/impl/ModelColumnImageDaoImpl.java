...
import com.newhc.admin.dao.ModelColumnImageDao;
import com.newhc.admin.entity.ModelColumnImage;


public class ModelColumnImageDaoImpl extends BaseDaoImpl implements ModelColumnImageDao {

	public void addModelColumnImage(ModelColumnImage modelColumnImage) {
		super.getHibernateTemplate().save(modelColumnImage);
	}

	public void deleteModelColumnImage(ModelColumnImage modelColumnImage) {
		super.getHibernateTemplate().delete(modelColumnImage);
	}
	...
	public Pagination queryModelColumnImagePage(String title, Integer status, Integer type,Integer parentType, Integer pageMaxSize, Integer pageNumber) {
		...
		query.append("select id,title,link,flash,path,parent_type,type,status,create_time,creator from model_column_image where 1=1 ");
	...
	public void updateModelColumnImage(ModelColumnImage modelColumnImage) {
		super.getHibernateTemplate().update(modelColumnImage);
	}

	public ModelColumnImage queryModelColumnImageById(Integer id) {
		return (ModelColumnImage)super.getHibernateTemplate().load(ModelColumnImage.class,id);
	}
...
//		query.append("rn >=1 AND rn<=? ");
		query.append("SELECT id,link,path,flash FROM model_column_image WHERE status = 1 AND  type = ? LIMIT ?");
		values.add(type);
		values.add(limit);
	...
	public List queryModelColumnImageBy(Integer userId) {
		...
		StringBuffer query = new StringBuffer();
		List values = new ArrayList();
		query.append("select id,user_id,title,link,path,status from model_column_image where parent_type = 2 and user_id = ? ");
		values.add(userId);
		return super.getJdbcDao().getJdbcTemplate().query(query.toString(), values.toArray(), new MapObjectRowMapper());
	}
