...
public class ModelAlbumTypeAction extends JSONPrintAction {
	private static final long serialVersionUID = 1L;
	private static final Log logger = LogFactory.getLog(ModelAlbumTypeAction.class);
	...
	private String uploadImagePath = realPath +"admin/uploadImage/AlbumType/";
	private String dataImagePath = "admin/uploadImage/AlbumType/";
	
	
	
	private ModelAlbumType modelPeriodical = new ModelAlbumType();
	
	public String processModelAlbumTypePageAction(){
		...
		Pagination pi = DaoFactory.getModelAlbumTypeDao().queryModelAlbumTypePage(name, status, pageMaxSize, pageNumber);
		List l=new ArrayList();
		if(pi!=null&&pi.getTotalCount()>0){
			for(Iterator i = pi.getList().iterator();i.hasNext();){
				Map map = (Map)i.next();
				ModelAlbumTypeView mpv = new ModelAlbumTypeView();
	 ...
	 * @return
	 */
	public String processPrepareAddModelAlbumTypeAction(){
		Integer userId = super.getOnlineUserId();
		if(userId==null) throw new ServiceException(ErrorCode.INVALID_REQUEST);
		return SUCCESS;
	}
	...
	public String processAddModelAlbumTypeAction(){
		Integer userId = super.getOnlineUserId();
		if(userId==null) throw new ServiceException(ErrorCode.INVALID_REQUEST);
//		判断图片类型
		...
		DaoFactory.getModelAlbumTypeDao().addModelModelAlbumType(modelPeriodical);
	 ...
	 * @return
	 */
	public String processPrepareUpdateModelAlbumTypeAction(){
		Integer userId = super.getOnlineUserId();
		if(userId==null) throw new ServiceException(ErrorCode.INVALID_REQUEST);
		if(id==null){
			super.setActionResponse(RESPONSE_FAILURE);
		}
		modelPeriodical = DaoFactory.getModelAlbumTypeDao().queryModelAlbumTypeById(id);
		return SUCCESS;
	}
	/**
	 * 修改
	 * @return
	 */
	public String processUpdateModelAlbumTypeAction(){
		Integer userId = super.getOnlineUserId();
		if(userId==null) throw new ServiceException(ErrorCode.INVALID_REQUEST);
		ModelAlbumType mp = DaoFactory.getModelAlbumTypeDao().queryModelAlbumTypeById(id);
//		判断图片类型
		if(fileImage!=null&&fileImageFileName!=null){
			boolean isSuitType = ImageHelperUtil.isSuitType(fileImageFileName);
			if(!isSuitType){
		...
		DaoFactory.getModelAlbumTypeDao().updateModelAlbumType(mp);
		super.setActionResponse(RESPONSE_SUCCESS);
		return execJSONReponseAction();
	}
	/**
	 ...
	 * @return
	 */
	public String processAuditModelAlbumTypeAction(){
		Integer userId = super.getOnlineUserId();
		if(userId==null) throw new ServiceException(ErrorCode.INVALID_REQUEST);
		if(id==null){
			super.setActionResponse(RESPONSE_FAILURE);
	 ...
	 * @return
	 */
	public String processReAuditModelAlbumTypeAction(){
		Integer userId = super.getOnlineUserId();
		if(userId==null) throw new ServiceException(ErrorCode.INVALID_REQUEST);
		if(id==null){
			super.setActionResponse(RESPONSE_FAILURE);
	 ...
	 * @return
	 */
	public String processAuditAllModelAlbumTypeAction(){
		String shenheId=ShenHeId.substring(0,ShenHeId.length()-1);
	    String[] shenhe=shenheId.split(",");
	    ModelAlbumType mp =null;
		try {
			if(shenheId==null){
				super.setActionResponse(RESPONSE_FAILURE);
			}
			for(int i=0;i<shenhe.length;i++){
				mp=DaoFactory.getModelAlbumTypeDao().queryModelAlbumTypeById(Integer.valueOf(shenhe[i]));
				mp.setStatus(new Integer(1));
				DaoFactory.getModelAlbumTypeDao().updateModelAlbumType(mp);
			}
			super.setActionResponse(RESPONSE_SUCCESS);
	 ...
	 * @return
	 */
	public String processReAuditAllModelAlbumTypeAction(){
		String shenheId=ShenHeId.substring(0,ShenHeId.length()-1);
	    String[] shenhe=shenheId.split(",");
	    ModelAlbumType mp =null;
		try {
	...
	public String processDeleteAllModelNewsAction(){
		...
		try {
			...
			for(int i=0;i<shenhe.length;i++){
				...
				mp =DaoFactory.getModelAlbumTypeDao().queryModelAlbumTypeById(shId);
				DaoFactory.getModelAlbumTypeDao().deleteModelAlbumType(mp);
			}
			super.setActionResponse(RESPONSE_SUCCESS);
		} catch (SecurityException e) {
			super.setActionResponse(RESPONSE_FAILURE);
	 ...
	 * @return
	 */
	public String processDeleteModelAlbumTypeAction(){
		Integer userId = super.getOnlineUserId();
		if(userId==null) throw new ServiceException(ErrorCode.INVALID_REQUEST);
		if(id==null){
			super.setActionResponse(RESPONSE_FAILURE);
	...
	public ModelAlbumType getModelPeriodical() {
		return modelPeriodical;
	}
	public void setModelPeriodical(ModelAlbumType modelPeriodical) {
