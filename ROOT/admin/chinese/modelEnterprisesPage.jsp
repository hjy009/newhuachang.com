...
<%@ page import="com.newhc.admin.action.user.view.ModelEnterprisesView"%>
...
<script language="javascript">
	function go2Page(page){
		...
		form.action = "modelEnterprisesPage.do?columnsType=<ww:property value='columnsType' />&typeName=<ww:property value='typeName' />";
		form.submit();
	}
	
	function prepareAddColumnImage(){
		<auth:if test="/admin/prepareAddModelEnterprises.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareAddModelEnterprises.do?columnsType=<ww:property value='columnsType' />&typeName=<ww:property value='typeName' />","新增<ww:property value='typeName' />",950,300) =="SUCCESS") refresh();
	}
	
	function prepareEditColumnImage(id){
	<auth:if test="/admin/prepareUpdateModelEnterprises.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareUpdateModelEnterprises.do?id="+id+"&typeName=<ww:property value='typeName' />","修改<ww:property value='typeName' />",950,300) =="SUCCESS") refresh();
	}
	
	function auditModelEnterprises(id,count){
	<auth:if test="/admin/auditModelEnterprises.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否审核此动态id：["+count+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'auditModelEnterprises.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	
	function reAuditModelEnterprises(id,count){
	<auth:if test="/admin/reAuditModelEnterprises.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否重审核此动态id：["+count+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'reAuditModelEnterprises.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	
	
	function deleteModelEnterprises(id,count){
		<auth:if test="/admin/deleteModelEnterprises.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm('是否删除此动态[id:'+count+']?')==false){
			return ;
		}
		var params = '';
		var url ="deleteModelEnterprises.do?id="+id;
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  shenhe(){
	   <auth:if test="/admin/auditAllModelEnterprises.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	...
	   	var ary ="";
			...
			var url = 'auditAllModelEnterprises.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	  function  reShenhe(){
	   <auth:if test="/admin/reAuditAllModelEnterprises.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	  	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = '';
			var url = 'reAuditAllModelEnterprises.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function  deleteAll(){
	   <auth:if test="/admin/deleteAllModelEnterprises.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = 'ShenHeId='+ary;
			var url = 'deleteAllModelEnterprises.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
