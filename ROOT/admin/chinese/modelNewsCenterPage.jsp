...
<%@ page import=" com.newhc.admin.action.user.view.ModelNewsCenterView"%>
...
<script language="javascript">
	function go2Page(page){
		...
		form.action = "modelNewsCenterPage.do";
		form.submit();
	}
	...
	//<auth:if test="/admin/prepareUpdateNewsCenter.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("modelNewsPage.do?centerId="+centerId+"&centerName="+name,name,1050,400) =="SUCCESS") refresh();
	}
	function prepareUpdateNewsCenter(id){
	<auth:if test="/admin/prepareUpdateModelNewsCenter.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareUpdateModelNewsCenter.do?id="+id,"修改新闻中心",850,300) =="SUCCESS") refresh();
	}
	
	function prepareAddNewsCenter(){
	<auth:if test="/admin/prepareAddModelNewsCenter.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareAddModelNewsCenter.do","新增新闻中心",850,300)=="SUCCESS")refresh();
	}
	
	function deleteNewsCenter(id,pageId){
	<auth:if test="/admin/deleteModelNewsCenter.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否删除此动态id：["+pageId+"] ？")==false){
			return ;
		}
		var params = '';
		var url ="deleteModelNewsCenter.do?id="+id;
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}

	function auditNewsCenter(id,pageId){
	<auth:if test="/admin/auditModelNewsCenter.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否审核此动态id：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'auditModelNewsCenter.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	
	function reAuditNewsCenter(id,pageId){
	<auth:if test="/admin/reAuditModelNewsCenter.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否重审核此动态id：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'reAuditModelNewsCenter.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  shenhe(){
	   <auth:if test="/admin/auditAllModelNewsCenter.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	...
	   	var ary ="";
			...
			var url = 'auditAllModelNewsCenter.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function reShenhe(){
		<auth:if test="/admin/reAuditAllModelNewsCenter.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = '';
			var url = 'reAuditAllModelNewsCenter.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function  deleteAll(){
	   <auth:if test="/admin/deleteAllNewsCenter.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = 'ShenHeId='+ary;
			var url = 'deleteAllNewsCenter.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
  ...
  /**
	...
	*/
	...
	if(openDialog("/chinese/news/newsWebPages.do?centerId="+id,"预览前台页面",1250,700)=="SUCCESS")refresh();
