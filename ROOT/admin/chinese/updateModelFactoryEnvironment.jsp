...
<script>
	...
	function addColumnImage(){
		...
		form.action = "updateFactoryEnvironment.do";
		form.submit();	
...
<form id="form" action="" method="POST" enctype="multipart/form-data">
	<input type="hidden" name = "id" value="<ww:property value='modelFactoryEnvironment.id'/>">
  <tr>
    <th><font color="#FF0000">*</font>名称：</th>
    <td>&nbsp;<input id="title" name=modelFactoryEnvironment.name value="<ww:property value='modelFactoryEnvironment.name'/>" size="50">&nbsp;<font color="#FF0000">(标题不能超过30个文字)</font></td>
  </tr>
  ...
  <tr id="imageTr">
    ...
    <td>&nbsp;<input type="file" id="fileMinImage" name="fileMinImage" value="<ww:property value='modelFactoryEnvironment.minImage'/>" size="30">
	 <ww:if test="modelFactoryEnvironment.minImage==null">图片：暂无</ww:if><ww:elseif test="modelFactoryEnvironment.minImage!=null">图片：<ww:property value='modelFactoryEnvironment.minImage'/><input type="button" value="预览" onClick="javascript:previewImageVideo(1,'<ww:property value="modelFactoryEnvironment.minImage"/>');" /></ww:elseif>
	<br><font color="#FF0000">(上传的图片只支持jpg,gif,bmp格式,大小不能超过1M)</font></td>
  </tr>
 <tr>
  	<th><font color="#FF0000">*</font>顺序：</th>
  	<td>&nbsp;<input id="sequence" name="modelFactoryEnvironment.sequence" value="<ww:if test='modelFactoryEnvironment.sequence==null'>1</ww:if><ww:elseif test='modelFactoryEnvironment.sequence!=null'><ww:property value='modelFactoryEnvironment.sequence'/></ww:elseif>" size="30"><font color="#FF0000">*注意:此框只能输入数字，顺序应从1开始，发布日期相同时,数字越大排越后</font></td>
  </tr>
