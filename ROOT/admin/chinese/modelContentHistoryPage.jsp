...
<%
	...
	List modelContentHistoryStatus = (List)request.getAttribute("modelContentHistoryStatus");
%>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=gbk">
<title>栏目内容历史管理</title>
<%@ include file="/WEB-INF/inc/default.jsp"%>
<script language="javascript">
	function go2Page(page){
		var form = findById("form");
		form.pageNumber.value = page;
		form.action = "modelContentHistoryPage.do";
		form.submit();
	}
	function viewModelContentHistory(id){
	<auth:if test="/admin/viewModelContentHistory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("viewModelContentHistory.do?id="+id,"查看栏目内容历史",850,300) =="SUCCESS") refresh();
	}
	
	function deleteModelContentHistory(id,pageId){
	<auth:if test="/admin/deleteModelContentHistory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm('是否删除此ID：['+pageId+']?')==false){
			return ;
		}
		var params = '';
		var url ="deleteModelContentHistory.do?id="+id;
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  deleteAll(){
	   <auth:if test="/admin/deleteAllModelContentHistory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	...
	   	var ary ="";
		    ...
		    if(ary.length<1) {
				alert("至少选择一个栏目内容历史进行删除");
				return;
			}
			
			if(confirm("是否批量删除您所选的选项 ？") == false) return ;
			var params = 'ShenHeId='+ary;
			var url = 'deleteAllModelContentHistory.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function recoveryModelContentDetail(contentId,id,pageId){
		...
		var params = '';
		var url ="recoveryModelContentDetail.do?contentId="+contentId+"&id="+id;
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
...
<table width="96%" align="center" border="0"><tr><td><div class="title">栏目内容历史管理-<font color="#FF0000"><ww:property value="detailName"/></font></div></td></tr></table>
<table width="96%" cellspacing="1" cellpadding="1" align="center">
	<form id='form' action="" method="POST">
	  <ww:hidden name="'contentId'"/>
	  <ww:hidden name="'detailName'"/>
	  <ww:hidden name="'pageNumber'" />
	  <tr>
		<td style="border:0;" >
			&nbsp;标题：<input id="name" name="name" size="30" value="<ww:property value='name'/>">
			&nbsp;状态：<select id="status" name="status">
							<%if(modelContentHistoryStatus!=null&&modelContentHistoryStatus.size()>0){%>
							<option value="">全部</option>
								<%for(Iterator it = modelContentHistoryStatus.iterator();it.hasNext();){
  ...
  <%
  	if(pages != null && pages.getTotalCount() > 0){
		...
		if(pageNumber>1){
	  	...
	  	for(Iterator it = pages.iterator();it.hasNext();){
	  	    count++;
			showCount++;
	  		ModelContentHistory sgv = (ModelContentHistory)it.next();
	...
	%>
			  <tr class="<%=rowClass%>" align="center" id="row_<%=showCount-1%>">
				...
				<%}else{%><font color="#FF0000"><%=DictionaryCache.getValueByCode(Dictionary.AUDIT_STATUS,sgv.getStatus()+"")%></font>
				<%}%>&nbsp;</td>
				...
				<td align="left">
				<input type="button" value="查看..." onClick="javascript:viewModelContentHistory(<%=sgv.getId()%>);">
				<input type="button" value="恢复..." onClick="javascript:recoveryModelContentDetail(<ww:property value='contentId'/>,<%=sgv.getId()%>,<%=count%>);">
				<input type="button" value="删除..." onClick="javascript:deleteModelContentHistory(<%=sgv.getId()%>,<%=count%>);">
				</td>
