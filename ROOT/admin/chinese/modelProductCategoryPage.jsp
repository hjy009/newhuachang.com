...
<%@ page import="com.newhc.admin.entity.ModelProductCategory"%>
...
<script language="javascript">
	function go2Page(page){
		...
		form.action = "modelProductCategoryPage.do?columnsType=<ww:property value='columnsType' />&typeName=<ww:property value='typeName' />";
		form.submit();
	}
	function prepareUpdateModelProductCategory(id){
		<auth:if test="/admin/prepareUpdateModelProductCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareUpdateModelProductCategory.do?id="+id+"&typeName=<ww:property value='typeName' />","修改<ww:property value='typeName' />分类",650,300) =="SUCCESS") refresh();
	}
	
	function prepareAddModelProductCategory(columnsType){
		<auth:if test="/admin/prepareAddModelProductCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareAddModelProductCategory.do?columnsType="+columnsType,"新增<ww:property value='typeName' />分类",650,200)=="SUCCESS")refresh();
	}
	
	function deleteModelProductCategory(id,pageId){
		<auth:if test="/admin/deleteModelProductCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm('是否删除此ID：['+pageId+']?')==false){
			return ;
		}
		var params = '';
		var url ="deleteModelProductCategory.do?id="+id;
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}

	function auditModelProductCategory(id,pageId){
		<auth:if test="/admin/auditModelProductCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否审核此动态ID：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'auditModelProductCategory.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	
	function reAuditModelProductCategory(id,pageId){
		<auth:if test="/admin/reAuditModelProductCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否重审核此动态ID：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'reAuditModelProductCategory.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  shenhe(){
	   <auth:if test="/admin/auditAllModelProductCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	...
	   	var ary ="";
			...
			var params = '';
			var url = 'auditAllModelProductCategory.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function  reShenhe(){
	   <auth:if test="/admin/reAuditAllModelProductCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = '';
			var url = 'reAuditAllModelProductCategory.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function modelProductPage(categoryId,categoryName,columnsType){
		<auth:if test="/admin/modelProductPage.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("modelProductPage.do?categoryId="+categoryId+"&categoryName="+categoryName+"&columnsType="+columnsType,categoryName,950,300) =="SUCCESS") refresh();
	}
</script>
</head>
<body>
