...
<%@ page import="com.newhc.admin.action.user.view.ModelVideoCategoryView"%>
...
<script language="javascript">
	function go2Page(page){
		...
		form.action = "modelVideoCategoryPage.do";
		form.submit();
	}
	function modelVideoPage(id){
		<auth:if test="/admin/modelVideoPage.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("modelVideoPage.do?categoryId="+id,"视频信息",1050,300)=="SUCCESS")refresh();
	}
	
	function prepareUpdateModelVideoCategory(id){
	<auth:if test="/admin/prepareUpdateModelVideoCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareUpdateModelVideoCategory.do?id="+id,"修改视频分类",550,300) =="SUCCESS") refresh();
	}
	
	function prepareAddModelVideoCategory(){
	<auth:if test="/admin/prepareAddModelVideoCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareAddModelVideoCategory.do","新增视频分类",550,300)=="SUCCESS")refresh();
	}
	function deleteModelVideoCategory(id,pageId){
	<auth:if test="/admin/deleteModelVideoCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm('是否删除此ID：['+pageId+']?')==false){
			return ;
		}
		var params = '';
		var url ="deleteModelVideoCategory.do?id="+id;
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	function auditModelVideoCategory(id,pageId){
	<auth:if test="/admin/auditModelVideoCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否审核此动态ID：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'auditModelVideoCategory.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	function reAuditModelVideoCategory(id,pageId){
	<auth:if test="/admin/reAuditModelVideoCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否重审核此动态ID：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'reAuditModelVideoCategory.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	function init(){
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  shenhe(){
	   <auth:if test="/admin/updateAllModelVideoCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	...
	   	var ary ="";
			...
			var url = 'updateAllModelVideoCategory.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   function  reShenhe(){
	   <auth:if test="/admin/reUpdateAllModelVideoCategory.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = 'ShenHeId='+ary;
			var url = 'reUpdateAllModelVideoCategory.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
  	...
  	if(pages != null && pages.getTotalCount() > 0){
		...
		if(pageNumber>1){
	  	...
	  	for(Iterator it = pages.iterator();it.hasNext();){
	  	    count++;
			showCount++;
	  		ModelVideoCategoryView videoCategory = (ModelVideoCategoryView)it.next();
