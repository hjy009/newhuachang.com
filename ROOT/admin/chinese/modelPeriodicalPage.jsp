...
<%@ page import="com.newhc.admin.action.user.view.ModelPeriodicalView"%>
...
<script language="javascript">
	function go2Page(page){
		...
		form.action = "modelPeriodicalPage.do?typeId=<ww:property value='typeId' />";
		form.submit();
	}
	function prepareUpdateModelPeriodical(id){
		<auth:if test="/admin/prepareUpdateModelPeriodical.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareUpdateModelPeriodical.do?id="+id+"&typeName=<ww:property value='typeName' />","修改<ww:property value='typeName' />分类",650,300) =="SUCCESS") refresh();
	}
	
	function prepareAddModelPeriodica(){
		<auth:if test="/admin/prepareAddModelPeriodical.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareAddModelPeriodical.do?typeId=<ww:property value='typeId'/>","新增<ww:property value='typeName' />分类",650,200)=="SUCCESS")refresh();
	}
	
	function deleteModelPeriodical(id,pageId){
		<auth:if test="/admin/deleteModelPeriodical.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm('是否删除此ID：['+pageId+']?')==false){
			return ;
		}
		var params = '';
		var url ="deleteModelPeriodical.do?id="+id;
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}

	function auditModelPeriodical(id,pageId){
		<auth:if test="/admin/auditModelPeriodical.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否审核此动态ID：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'auditModelPeriodical.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	
	function reAuditModelPeriodical(id,pageId){
		<auth:if test="/admin/reAuditModelPeriodical.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否重审核此动态ID：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'reAuditModelPeriodical.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  shenhe(){
	   <auth:if test="/admin/auditAllModelPeriodical.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	...
	   	var ary ="";
			...
			var params = '';
			var url = 'auditAllModelPeriodical.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function  reShenhe(){
	   <auth:if test="/admin/reAuditAllModelPeriodical.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = '';
			var url = 'reAuditAllModelPeriodical.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	    function  deleteAll(){
	   <auth:if test="/admin/deleteAllModelPeriodical.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var url = 'deleteAllModelPeriodical.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   function modelProductPage(categoryId){
		<auth:if test="/admin/modelPeriodicalCenterPage.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("modelPeriodicalCenterPage.do?periodicalId="+categoryId,"电子期刊内容管理",950,300) =="SUCCESS") refresh();
	}
</script>
</head>
<body>
