...
<script language="javascript">
	function go2Page(page){
		...
		form.action = "modelAdvertisePage.do";
		form.submit();
	}
	function queryModelAdvertise(id){
	<auth:if test="/admin/queryModelAdvertise.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("queryModelAdvertise.do?id="+id,"查看招聘信息",550,300) =="SUCCESS") refresh();
	}
	function prepareAddModelAdvertise(){
	<auth:if test="/admin/prepareAddModelAdvertise.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareAddModelAdvertise.do","新增招聘信息",850,300) =="SUCCESS") refresh();
	}
	function prepareUpdateModelAdvertise(id){
	<auth:if test="/admin/prepareUpdateModelAdvertise.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareUpdateModelAdvertise.do?id="+id,"修改招聘信息",850,300) =="SUCCESS") refresh();
	}
	function auditModelAdvertise(id,pageId){
	<auth:if test="/admin/auditModelAdvertise.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否审核此动态id：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'auditModelAdvertise.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	
	function reAuditModelAdvertise(id,pageId){
	<auth:if test="/admin/reAuditModelAdvertise.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否重审核此动态id：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'reAuditModelAdvertise.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	function deleteModelAdvertise(id,pageId){
	<auth:if test="/admin/deleteModelAdvertise.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否删除此动态id：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'deleteModelAdvertise.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  shenhe(){
	   <auth:if test="/admin/updateAllModelAdvertise.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	...
	   	var ary ="";
			...
			var url = 'updateAllModelAdvertise.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   function  reShenhe(){
	   <auth:if test="/admin/reUpdateAllModelAdvertise.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = 'ShenHeId='+ary;
			var url = 'reUpdateAllModelAdvertise.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	function  deleteAll(){
	   <auth:if test="/admin/deleteAllModelAdvertise.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = 'ShenHeId='+ary;
			var url = 'deleteAllModelAdvertise.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	function init(){
