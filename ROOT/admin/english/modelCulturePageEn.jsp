...
<%@ page import="com.newhc.admin.action.user.view.en.ModelCultureEnView"%>
...
<script language="javascript">
	function go2Page(page){
		...
		form.action = "modelCultureEnPage.do";
		form.submit();
	}
	function prepareUpdateModelCulture(id){
	<auth:if test="/admin/prepareUpdateModelCultureEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareUpdateModelCultureEn.do?id="+id,"修改企业文化",850,300) =="SUCCESS") refresh();
	}
	
	function prepareAddModelCulture(){
	<auth:if test="/admin/prepareAddModelCultureEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareAddModelCultureEn.do","新增企业文化",850,300)=="SUCCESS")refresh();
	}
	
	function deleteModelCulture(id,pageId){
	<auth:if test="/admin/deleteModelCultureEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否删除此动态id：["+pageId+"] ？")==false){
			return ;
		}
		var params = '';
		var url ="deleteModelCultureEn.do?id="+id;
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}

	function auditModelCulture(id,pageId){
	<auth:if test="/admin/auditModelCultureEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否审核此动态id：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'auditModelCultureEn.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	
	function reAuditModelCulture(id,pageId){
	<auth:if test="/admin/reAuditModelCultureEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否重审核此动态id：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'reAuditModelCultureEn.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  shenhe(){
	   <auth:if test="/admin/updateAllModelCultureEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   ...
	   function reShenhe(){
	    <auth:if test="/admin/reUpdateAllModelCultureEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	    var objs = document.getElementsByName("productId");
	    var ary = "";
	    for(i=0;i<objs.length;i++){
		    if(objs[i].checked){
		...
		var params = 'ShenHeId='+ary;
		var url = 'reUpdateAllModelCultureEn.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   
	   function  deleteAll(){
	   <auth:if test="/admin/deleteAllModelCultureEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = 'ShenHeId='+ary;
			var url = 'deleteAllModelCultureEn.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
...
<table width="96%"  class="tborder" border="0" cellspacing="0" cellpadding="0" align="center">
  ...
  %>
			  <tr class="<%=rowClass%>" align="center" id="row_<%=showCount-1%>">
				...
				<td align="left">
				<input type="button" value="修改...<%=miv.getId()%>" onClick="javascript:prepareUpdateModelCulture(<%=miv.getId()%>);">
