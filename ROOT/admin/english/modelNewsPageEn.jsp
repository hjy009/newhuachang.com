...
<%@ page import="com.newhc.admin.action.user.view.en.ModelNewsEnView"%>
...
<script language="javascript">
	function go2Page(page){
		...
		form.action = "modelNewsPageEn.do?centerId=<ww:property value='centerId' />";
		form.submit();
	}
	function prepareUpdateModelNews(id){
	<auth:if test="/admin/prepareUpdateModelNewsEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareUpdateModelNewsEn.do?id="+id,"修改新闻发布",850,300) =="SUCCESS") refresh();
	}
	
	function prepareAddModelNews(){
	<auth:if test="/admin/prepareAddModelNewsEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareAddModelNewsEn.do?centerId=<ww:property value='centerId' />","新增新闻发布",850,300)=="SUCCESS")refresh();
	}
	
	function deleteModelNews(id,pageId){
	<auth:if test="/admin/deleteModelNewsEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm('是否删除此ID：['+pageId+']？删除后将找不回此新闻关联的历史记录')==false){
			return ;
		}
		var params = '';
		var url ="deleteModelNewsEn.do?id="+id;
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}

	function auditModelNews(id,pageId){
	<auth:if test="/admin/auditModelNewsEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否审核此动态ID：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'auditModelNewsEn.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	
	function reAuditModelNews(id,pageId){
	<auth:if test="/admin/reAuditModelNewsEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否重审核此动态ID：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'reAuditModelNewsEn.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  shenhe(){
	   <auth:if test="/admin/auditAllModelNewsEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	...
	   	var ary ="";
			...
			var params = '';
			var url = 'auditAllModelNewsEn.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function  reShenhe(){
	   <auth:if test="/admin/reAuditAllModelNewsEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = '';
			var url = 'reAuditAllModelNewsEn.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function  deleteAll(){
	   <auth:if test="/admin/deleteAllModelNewsEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var url = 'deleteAllModelNewsEn.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	function modelNewsHistoryPage(newsId,newsTitle){
		<auth:if test="/admin/modelNewsHistoryPageEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("modelNewsHistoryPageEn.do?newsId="+newsId+"&newsTitle="+newsTitle,"新闻历史管理",950,300)=="SUCCESS")refresh();
	}
</script>
</head>
<body>
