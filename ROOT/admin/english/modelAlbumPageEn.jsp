...
<%@ page import="com.newhc.admin.action.user.view.en.ModelAlbumEnView"%>
...
<script language="javascript">
	function go2Page(page){
		...
		form.action = "modelAlbumEnPage.do?typeId=<ww:property value='typeId' />";
		form.submit();
	}
	function prepareUpdateModelAlbum(id){
		<auth:if test="/admin/prepareUpdateModelAlbumEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareUpdateModelAlbumEn.do?id="+id+"&typeName=<ww:property value='typeName' />","修改电子画册",650,300) =="SUCCESS") refresh();
	}
	
	function prepareAddModelPeriodica(){
		<auth:if test="/admin/prepareAddModelAlbumEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareAddModelAlbumEn.do?typeId=<ww:property value='typeId'/>","新增电子画册",650,200)=="SUCCESS")refresh();
	}
	
	function deleteModelAlbum(id,pageId){
		<auth:if test="/admin/deleteModelAlbumEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	...
	function auditModelAlbum(id,pageId){
		<auth:if test="/admin/auditModelAlbumEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	...
	function reAuditModelAlbum(id,pageId){
		<auth:if test="/admin/reAuditModelAlbumEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  shenhe(){
	   <auth:if test="/admin/auditAllModelAlbumEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	...
	   	var ary ="";
			...
			var params = '';
			var url = 'auditAllModelAlbumEn.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function  reShenhe(){
	   <auth:if test="/admin/reAuditAllModelAlbumEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = '';
			var url = 'reAuditAllModelAlbumEn.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	    function  deleteAll(){
	   <auth:if test="/admin/deleteAllModelAlbumEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
