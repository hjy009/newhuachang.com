...
<%@ page import="com.newhc.admin.action.user.view.en.ModelVideoEnView"%>
...
<script language="javascript">
	function go2Page(page){
		...
		form.action = "modelVideoEnPage.do";
		form.submit();
	}
	function prepareUpdateModelVideo(id){
	<auth:if test="/admin/prepareUpdateModelVideoEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareUpdateModelVideoEn.do?id="+id,"修改视频信息",550,300) =="SUCCESS") refresh();
	}
	
	function prepareAddModelVideo(categoryId){
	<auth:if test="/admin/prepareAddModelVideoEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareAddModelVideoEn.do?categoryId="+categoryId,"新增视频信息",550,300)=="SUCCESS")refresh();
	}
	function deleteModelVideo(id,pageId){
	<auth:if test="/admin/deleteModelVideoEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm('是否删除此ID：['+pageId+']?')==false){
			return ;
		}
		var params = '';
		var url ="deleteModelVideoEn.do?id="+id;
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	function auditUserGardenNews(id,pageId){
	<auth:if test="/admin/auditModelVideoEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否审核此动态ID：["+pageId+"] ？") == false) return ;
		var params = 'id ='+id;
		var url = 'auditModelVideoEn.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	
	function reAuditUserGardenNews(id,pageId){
	<auth:if test="/admin/reAuditModelVideoEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否重审核此动态ID：["+pageId+"] ？") == false) return ;
		var params = 'id ='+id;
		var url = 'reAuditModelVideoEn.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	function downloadVideo(id){
		//var params = '';
		//window.location.href="downloadVideo.do?id="+id;
		top.location="downloadVideoEn.do?id="+id;
	}
	function init(){
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  shenhe(){
	   <auth:if test="/admin/updateAllModelVideoEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	...
	   	var ary ="";
		    ...
		    if(ary.length<1) {
				alert("至少选择一个视频信息进行审核");
				return;
			}
			
			if(confirm("是否批量审核您所选的选项 ？") == false) return ;
			var params = 'ShenHeId='+ary;
			var url = 'updateAllModelVideoEn.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   function  reShenhe(){
	   <auth:if test="/admin/reUpdateAllModelVideoEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
		    ...
		    if(ary.length<1) {
				alert("至少选择一个视频信息进行重审核");
				return;
			}
			if(confirm("是否批量重审核您所选的选项 ？") == false) return ;
			var params = 'ShenHeId='+ary;
			var url = 'reUpdateAllModelVideoEn.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   function  deleteAll(){
	   <auth:if test="/admin/deleteAllModelVideoEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
		    ...
		    if(ary.length<1) {
				alert("至少选择一个视频信息进行删除");
				return;
			}
			if(confirm("是否批量删除您所选的选项 ？") == false) return ;
			var params = 'ShenHeId='+ary;
			var url = 'deleteAllModelVideoEn.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
</script>
</head>
<body>
<table width="96%" align="center" border="0"><tr><td><div class="title">视频信息</div></td></tr></table>
<table width="96%" cellspacing="1" cellpadding="1" align="center">
	<form id='form' action="" method="POST">
	  <ww:hidden name="'categoryId'" />
	  <tr>
		<td style="border:0;" >
			&nbsp;日期从：<ww:textfield name="'startT'" id="'startT'" size="'9'" maxlength="10"/><img src='/images/admin/calendar.gif' alt='点击选择日期' border='0' align='absmiddle' style="cursor:hand" onClick="return showCalendar('startT');">
			到：<ww:textfield name="'endT'" size="'9'" maxlength="10"/><img src='/images/admin/calendar.gif' alt='点击选择日期' border='0' align='absmiddle' style="cursor:hand" onClick="return showCalendar('endT');">
			&nbsp;名称：<input id="name" name="name" size="15" value="<ww:property value='title'/>">
			...
			&nbsp;&nbsp;<input type="button" name="addmp" value="新增视频信息..." class="mainoption" onClick="javascript:prepareAddModelVideo(<ww:property value='categoryId'/>);">
	...
	%>
			  <tr class="<%=rowClass%>" align="center" id="row_<%=showCount-1%>">
			   <td width="3%"><input type="checkbox" name="productId" value="<%=sgv.getId()%>"></td>
				...
				<td><%=StringHelper.isEmpty(sgv.getPath())?"---":sgv.getPath() %><input  type="button" value="下载..." onClick="javascript:downloadVideo(<%=sgv.getId()%>);"></td>
				...
				<td align="left">
				<input type="button"value="修改..."onClick="javascript:prepareUpdateModelVideo(<%=sgv.getId()%>);">
				<input type="button" value="删除..." onClick="javascript:deleteModelVideo(<%=sgv.getId()%>,<%=count%>);">
				</td>
