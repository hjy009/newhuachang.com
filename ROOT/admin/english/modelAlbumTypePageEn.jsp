...
<%@ page import="com.newhc.admin.action.user.view.en.ModelAlbumTypeEnView"%>
...
<script language="javascript">
	...
	function prepareUpdateModelAlbumTypeView(id){
		<auth:if test="/admin/prepareUpdateModelAlbumTypeEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareUpdateModelAlbumTypeEn.do?id="+id+"&typeName=<ww:property value='typeName' />","修改电子画册分类",650,300) =="SUCCESS") refresh();
	}
	
	function prepareAddModelPeriodica(){
		<auth:if test="/admin/prepareAddModelAlbumTypeEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareAddModelAlbumTypeEn.do","新增电子画册分类",650,200)=="SUCCESS")refresh();
	}
	
	function deleteModelAlbumTypeView(id,pageId){
		<auth:if test="/admin/deleteModelAlbumTypeEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm('是否删除此ID：['+pageId+']?')==false){
			return ;
		}
		var params = '';
		var url ="deleteModelAlbumTypeEn.do?id="+id;
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}

	function auditModelAlbumTypeView(id,pageId){
		<auth:if test="/admin/auditModelAlbumTypeEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	...
	function reAuditModelAlbumTypeView(id,pageId){
		<auth:if test="/admin/reAuditModelAlbumTypeEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  shenhe(){
	   <auth:if test="/admin/auditAllModelAlbumTypeEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	...
	   	var ary ="";
			...
			var params = '';
			var url = 'auditAllModelAlbumTypeEn.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function  reShenhe(){
	   <auth:if test="/admin/reAuditAllModelAlbumTypeEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = '';
			var url = 'reAuditAllModelAlbumTypeEn.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	    function  deleteAll(){
	   <auth:if test="/admin/deleteAllModelAlbumTypeEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var url = 'deleteAllModelAlbumTypeEn.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   function modelProductPage(categoryId){
		<auth:if test="/admin/modelAlbumEnPage.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("modelAlbumEnPage.do?typeId="+categoryId,"电子画册管理",950,300) =="SUCCESS") refresh();
	}
</script>
</head>
<body>
