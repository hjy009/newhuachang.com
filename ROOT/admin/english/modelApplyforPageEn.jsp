...
<%@ page import="com.newhc.admin.action.user.view.en.ModelApplyforEnView"%>
...
<script language="javascript">
	function go2Page(page){
		...
		form.action = "modelApplyforEnPage.do";
		form.submit();
	}
	function queryModelAdvertise(id){
		if(openDialog("queryModelApplyforEn.do?id="+id,"查看应聘信息",550,300) =="SUCCESS") refresh();
	}
	

	function deleteModelAdvertise(id,pageId){
	<auth:if test="/admin/deleteModelApplyforEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否删除此动态id：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'deleteModelApplyforEn.do';
	...
	function  deleteAll(){
	   <auth:if test="/admin/deleteAllModelApplyforEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	...
	<auth:if test="/admin/downloadFilesApplyforEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		var params = '';
