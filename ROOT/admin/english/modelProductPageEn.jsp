...
<%@ page import="com.newhc.admin.entity.en.ModelProductEn"%>
...
<script language="javascript">
	function go2Page(page){
		...
		form.action = "modelProductEnPage.do?columnsType=<ww:property value='columnsType' />";
		form.submit();
	}
	function prepareUpdateModelProduct(id){
	<auth:if test="/admin/prepareUpdateModelProductEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareUpdateModelProductEn.do?id="+id+"&columnsType=<ww:property value='columnsType' />&categoryName=<ww:property value='categoryName'/>","修改<ww:property value='categoryName'/>",850,300) =="SUCCESS") refresh();
	}
	
	function prepareAddModelProduct(categoryId){
	<auth:if test="/admin/prepareAddModelProductEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("prepareAddModelProductEn.do?categoryId="+categoryId+"&columnsType=<ww:property value='columnsType' />&categoryName=<ww:property value='categoryName'/>","新增<ww:property value='categoryName'/>",850,200)=="SUCCESS")refresh();
	}
	
	function deleteModelProduct(id,pageId){
	<auth:if test="/admin/deleteModelProductEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm('是否删除此ID：['+pageId+']？删除后将找不回此产品关联的历史记录')==false){
			return ;
		}
		var params = '';
		var url ="deleteModelProductEn.do?id="+id;
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}

	function auditModelProduct(id,pageId){
	<auth:if test="/admin/auditModelProductEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否审核此动态ID：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'auditModelProductEn.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	
	function reAuditModelProduct(id,pageId){
	<auth:if test="/admin/reAuditModelProductEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(confirm("是否重审核此动态ID：["+pageId+"] ？") == false) return ;
		var params = 'id='+id;
		var url = 'reAuditModelProductEn.do';
		new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	}
	...
	function   SelectAll(){      
	 var  aa = document.getElementsByName("productId");      
	   ...
	   function  shenhe(){
	   <auth:if test="/admin/auditAllModelProductEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	...
	   	var ary ="";
			...
			var params = '';
			var url = 'auditAllModelProductEn.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function  reShenhe(){
	   <auth:if test="/admin/reAuditAllModelProductEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var params = '';
			var url = 'reAuditAllModelProductEn.do?ShenHeId='+ary;
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	   function  deleteAll(){
	   <auth:if test="/admin/deleteAllModelProductEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
	   	var objs=document.getElementsByName("productId");
	   	var ary ="";
		    for(i=0;i<objs.length;i++) {
		        if(objs[i].checked) {
			...
			var url = 'deleteAllModelProductEn.do';
			new Ajax.Request( url, { method: 'get', parameters: params, onComplete: commonReponse });
	   }
	   
	  function modelProductHistoryPage(productId,categoryName){
		<auth:if test="/admin/modelProductHistoryPageEn.do" status="false">alert('对不起，您无此权限，请与系统管理员联系!');return;</auth:if>
		if(openDialog("modelProductHistoryPageEn.do?productId="+productId+"&categoryName="+categoryName,"产品历史管理",900,300) =="SUCCESS") refresh();
	} 
</script>
</head>
<body>
